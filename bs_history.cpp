...
void beamSearchWithHistory(int start, int goal,
                           vector<vector<pair<int,int>>> &graph,
                           vector<int> &heuristic, int beamWidth) {
    vector<NodeState> frontier;
    unordered_set<int> visited; // history of visited nodes
...
int main() {
    ...
    int beamWidth = 2; // Keep only the best 2 candidates at each level
    beamSearchWithHistory(0, 5, graph, heuristic, beamWidth);
